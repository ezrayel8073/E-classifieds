<%-- 
    Document   : Carsviewtable
    Created on : Apr 4, 2024, 1:56:04 PM
    Author     : hp
--%>
    ...
    <body>
     ...
     <li class="active"> <a href="home2.jsp"><i class='bx bxs-home'></i> Home</a> </li> 
      ...
      <li>  <a href="Job.jsp">Jobs</a></li>
        ...
        <center>
        <h1 target="blank">Cars Details</h1>
...
try{
    ...
    ResultSet rs=st.executeQuery("select *from cars where Company_Name='"+s+"'");
    //if(name.equals("admin") & pass.equals("admin"))
        while(rs.next())
         {
 int id=rs.getInt("uid");
String a=rs.getString("Company_Name"); 
String b=rs.getString("Owners_Name");
String c=rs.getString("Color");
String d=rs.getString("Mileage");
String e=rs.getString("Fuel_Type");
String f=rs.getString("Top_Speed");
String g=rs.getString("Kilometers_Driven");
String h=rs.getString("Services_Done");
...
<td><%=i%></td>
<td><%=j%></td>
<td> <a href="carsupdatetable.jsp?uid=<%=id%>&Company_Name=<%=a%>&Owners_Name=<%=b%>&Color=<%=c%>&Mileage=<%=d%>&Fuel_Type=<%=e%>&Top_Speed=<%=f%>&Kilometers_Driven=<%=g%>&Services_Done=<%=h%>&Contact=<%=i%>&Date=<%=j%>"> update <i class='bx bxs-edit-alt'></i></td>
<td><a href="Carsviewtabledelete.jsp?uid=<%=id%>">delete <i class='bx bxs-trash-alt'></i></a></td>
</tr>
<%
}
}catch(Exception e)           
...
%><br>
        ...
        <div  class="button">
 <a href="carsresponse.jsp" class="btn">Back</a>
     </div>
    </body></center><br>
</html>
